...
| SUP-040 | Supabase / Validación / Borrados y auditoría | P1 | CONFIRMADO | El borrado de caja SYNCROLAB registra la eliminación antes del `DELETE` e ignora `res.ok`; el borrado de FIO también registra antes y trata como éxito el posible `null` de `dbDelete()`. Ambos pueden dejar el registro intacto con auditoría y mensaje de eliminación. La caja puede tener cargos relacionados cuya política de cascada no se comprueba. | `validacion.js:1026-1040` y `validacion.js:2277-2294`. Ambas funciones revalidan un rol administrativo y solicitan confirmación; cascada de `syncrolab_room_charges` y efectos LIVE: `[NO DATA]`. | Auditar sólo el resultado confirmado, exigir cardinalidad y definir borrado/restricción/cascada de cargos. Para FIO, valorar baja lógica o anulación auditada frente a eliminación física antes de corregir. |
...
| ID | Área | Prioridad | Estado | Problema o riesgo | Evidencia actual | Acción requerida |
|---|---|---:|---|---|---|---|
| SEC-001 | Supabase / RLS | P0 | CONFIRMADO | Las 51 tablas ordinarias de `public` tienen RLS activado, pero en 43 resulta ineficaz como barrera de seguridad porque existen policies anónimas de CRUD completo. | Consulta LIVE de solo lectura sobre `pg_class`: 51 tablas, 51 con RLS, ninguna sin RLS y ninguna con `FORCE ROW LEVEL SECURITY`. Cinco tablas quedan bloqueadas al no tener policies (`cocina_costes_mes`, `cuadrantes`, `dept_reports`, `employee_status`, `incentivos_liquidaciones`). Existe una vista, `hk_incidencias_detalle`, con `security_invoker=on`. | Diseñar la matriz de acceso objetivo e implantar identidad confiable y policies restrictivas por usuario, rol, ámbito y fila; preparar migración y reversión antes de modificar LIVE. |
| SEC-002 | Supabase / Policies | P0 | CONFIRMADO | Las policies LIVE de `public` son estructuralmente permisivas: 43 tablas permiten CRUD anónimo completo y tres permiten lectura anónima. | Consulta LIVE de `pg_policies`: 66 policies en `public`, todas `PERMISSIVE`; 62 dirigidas a `anon` o `public`; no existe ninguna expresión `USING` o `WITH CHECK` distinta de `true` entre las policies de `public`. CRUD anónimo completo confirmado por policy y privilegios en 43 tablas; lectura anónima en `escandallo_lineas`, `platos_carta` y `productos_compra`. | Sustituir las policies abiertas por reglas restrictivas vinculadas a una identidad de aplicación verificable; validar casos permitidos y denegados en un entorno controlado antes del cambio LIVE. |
...
| SEC-010 | Supabase / Datos sensibles | P0 | CONFIRMADO | La identidad anónima puede leer y modificar la tabla `employees` completa, que contiene PIN, coste, rol, validador, correo y datos organizativos. | Policy LIVE de `employees`: `ALL` para `anon, authenticated`, `USING true`, `WITH CHECK true`; grants de tabla `SELECT`, `INSERT`, `UPDATE` y `DELETE` para `anon`. La auditoría no reprodujo valores sensibles ni ejecutó mutaciones. | Tratar `employees` como dato crítico: retirar PIN y autorización del cliente, limitar columnas y filas desde backend/RLS y rotar los PIN privilegiados durante una contención aprobada. |
...
| SEC-013 | Autenticación / PIN | P0 | PLANIFICADO | El acceso por PIN no crea una identidad autenticada ni una sesión verificable: el navegador descarga `employees`, compara el PIN localmente y asigna el registro encontrado a la variable global `currentUser`. Además existen PIN de rol privilegiado codificados en el JavaScript entregado al cliente y reproducidos en documentación del repositorio, por lo que no pueden considerarse secretos. | `shared.js:121`, `shared.js:592-638`, `docs/context/03_roles_permissions.md:26` y `docs/context/mapa-modulos.md:43`; LIVE confirma acceso anónimo completo a `employees`. El 08/08/2026 se aprobaron selección de empleado + PIN individual de seis dígitos y entrega temporal híbrida. La implementación local desactivada incluye Supabase Auth, huella única, caducidad, versión de autorización y gestión server-side completa del maestro; 20 pruebas ordinarias y un E2E local pasan. La migración base fue aplicada dos veces y revertida con éxito en PostgreSQL 17.10 local; se verificaron RLS, grants, contexto, unicidad, rate limit, auditoría y colisión simulada de PIN. Un stack Supabase local con Postgres/Auth/PostgREST/Kong confirmó alta ficticia, login con PIN temporal, cambio obligatorio, revocación del PIN/sesión anteriores, nuevo login y HTTP 401 anónimo sobre `employees` e identidades; los puertos quedaron limitados a `127.0.0.1`. | Completar matriz RLS, protección de columnas y Storage; ampliar la integración ya superada al esquema operativo completo fuera de LIVE. Después, aplicar mediante un corte aprobado la identidad individual, retirar PINs y descarga de empleados del cliente, cerrar acceso `anon` y rotar credenciales privilegiadas. |
...
| Sin sesión / `anon` | Clave publicable del frontend → rol PostgreSQL `anon` | 43 tablas `public` enumeradas en `SEC-002` | PERMITIDO | PERMITIDO | PERMITIDO | PERMITIDO | Todas las filas | Sin restricción de columnas a nivel de tabla | `pg_policies` + `has_table_privilege()` LIVE | Autorización efectiva confirmada por catálogo; pruebas de escritura no ejecutadas | CONFIRMADO |
...
- No se ha desplegado ninguna corrección ni se ha modificado Supabase LIVE como resultado de este registro.
