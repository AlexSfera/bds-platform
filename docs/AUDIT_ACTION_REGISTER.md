...
| UI-005 | Interfaz / Adjuntos de gestiones | P1 | CORREGIDO | La autorización de adjuntos pedía `id,adjuntos,updated_at` en todas las tablas; una gestión legacy sin `updated_at` hacía fallar la consulta completa y se traducía incorrectamente a 403 `Forbidden`, impidiendo subir y mostrar imágenes aunque el registro fuera visible. | Tres respuestas 403 de `/api/attachments/sign-upload` observadas en los logs del Preview tras el reporte del 15/08/2026. `requireReadableRecord()` contenía la selección rígida; la comprobación se limita ahora a `select=id`, suficiente para que RLS acredite la lectura. Prueba de regresión local añadida; navegador Preview pendiente. | Desplegar en Preview y verificar subida, miniatura, reapertura y descarga de una imagen en Gestión antes de cambiar a `VERIFICADO`. |
