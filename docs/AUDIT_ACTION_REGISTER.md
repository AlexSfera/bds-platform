...
|---|---|---:|---|---|---|---|
| ARCH-001 | Arquitectura / Adjuntos | P1 | CORREGIDO | `adjuntos.js` se cargaba dos veces en `index.html`, provocando doble ejecución de wrappers, `MutationObserver` e inicialización. | La preparación local de Storage del 14/08/2026 retiró la segunda etiqueta después de auditar los consumidores; una prueba exige exactamente una carga. Integración real de navegador: `[NO DATA]`. | Verificar en Preview turnos, incidencias, gestiones y tareas antes de cambiar a `VERIFICADO`. |
...
| UI-004 | Interfaz / Validación operativa | P1 | CORREGIDO | En la tabla operativa de Validación sólo Incidencias mostraba un estado gestionable; Gestiones y Tareas renderizaban badges estáticos, por lo que el usuario no podía abrir sus acciones desde el estado. | Reporte de usuario en Preview del 15/08/2026 y revisión de `valGestRenderTable()` / `valTaskRenderTable()`. La corrección conecta Gestiones con su modal existente y añade a Tareas un modal limitado por los permisos de transición ya vigentes. Prueba estática y suite local superadas; navegador Preview pendiente. | Desplegar en Preview y verificar clic y transición permitida en los tres bloques antes de cambiar a `VERIFICADO`. |
| UI-005 | Interfaz / Adjuntos de gestiones | P1 | CORREGIDO | La autorización y el guardado de metadatos de adjuntos dependían de `updated_at`, columna que no está garantizada en gestiones legacy. La primera consulta se traducía incorrectamente a 403 `Forbidden`; tras corregirla, el objeto subía pero el `PATCH` de metadatos fallaba y activaba una limpieza compensatoria. | Tres 403 iniciales de `/api/attachments/sign-upload`; tras el primer fix, logs Preview con `sign-upload` 200 seguido de `DELETE /api/attachments/object` 503, patrón que sólo ejecuta el `catch` de `adjuntoSaveToRecord()`. La autorización usa ahora `select=id` y el `PATCH` actualiza únicamente `adjuntos`. Pruebas de regresión local añadidas; navegador Preview pendiente. | Desplegar en Preview y verificar subida, miniatura, reapertura y descarga de una imagen en Gestión antes de cambiar a `VERIFICADO`; revisar aparte el objeto huérfano posible mediante inventario, sin borrado automático. |
| UI-006 | Interfaz / Modal operativo de adjuntos | P1 | CORREGIDO | Los modales abiertos al pulsar el estado no montaban el visor de adjuntos de forma uniforme. Gestión e Incidencia compartían un modal sin visor; Tarea utilizaba otro modal también sin visor. Por ello una imagen guardada no podía verse allí y tampoco aparecían `Sin adjuntos` ni los avisos de carga. | Capturas de usuario del Preview del 15/08/2026 y revisión de `openItemModal()` / `openTaskStateModal()`: no existía bloque `data-adj-viewer`; los consumidores estaban sólo en detalle Dashboard y panel de la pantalla Tareas. Se añaden wrappers que montan, autorizan e inicializan el visor en ambos modales operativos. | Verificar en Preview Gestión, Incidencia y Tarea con y sin imagen, incluida reapertura y descarga, antes de cambiar a `VERIFICADO`. |
| UI-007 | Interfaz / Eliminación de adjuntos | P1 | CORREGIDO | El endpoint eliminaba primero el objeto y después exigía guardar el evento de auditoría. Si esa escritura secundaria fallaba, respondía 503; el cliente interpretaba que nada se había borrado y restauraba los metadatos, dejando una referencia rota hacia un objeto ya eliminado. | Secuencia Preview del 15/08/2026: dos `sign-download` 200, `DELETE /api/attachments/object` 503 y posterior `sign-download` 503. El handler esperaba `recordAttachmentEvent()` después de `deleteAttachmentObject()`. La auditoría pasa a mejor esfuerzo después del borrado y la eliminación de un objeto ausente se trata como idempotente. | Desplegar en Preview, retirar la referencia rota y subir una imagen nueva; verificar vista, descarga y eliminación antes de cambiar a `VERIFICADO`. |
...
| SUP-011 | Supabase Storage / Consistencia | P1 | CORREGIDO | La subida y eliminación de adjuntos no coordinaban el objeto de Storage con el array de metadatos del registro y podían comunicar éxito con objeto huérfano o referencia rota. | Preparación local del 14/08/2026: `adjuntoSaveToRecord()` exige resultado, la subida intenta retirar objetos si falla el metadato y la baja intenta restaurar el array si falla Storage. Las pruebas JavaScript y de sintaxis pasan; integración real de navegador/Storage: `[NO DATA]`. | Verificar el flujo completo en Preview con fallos inducidos antes de cambiar a `VERIFICADO` o aplicar la migración LIVE. |
...
| SEC-012 | Supabase Storage / Exposición | P0 | CONFIRMADO | El bucket `adjuntos` continúa documentado como público y con acceso amplio; el estado administrativo LIVE actual no pudo reconfirmarse porque el conector disponible no incluye el proyecto. | Evidencia LIVE del 08/08/2026: bucket público, sin límites, siete policies amplias y descarga anónima. El 14/08/2026 los cuatro recursos REST de control siguieron devolviendo 401 anónimo, pero esa comprobación no prueba Storage. Preparación local `20260814192801_p0_private_attachments.sql`, rollback, endpoints firmados, límites MIME/tamaño y retirada de EXIF; doble aplicación y rollback SQL verificados. | Ejecutar el preflight de `docs/P0_STORAGE_ATTACHMENTS.md`, verificar Preview con Storage real y aplicar el corte LIVE únicamente después de confirmar compatibilidad y rollback. |
...
| SEC-036 | API / Backfill Bitrix | P1 | CORREGIDO | El endpoint nuevo de backfill histórico aceptaba cualquier método HTTP y su alternativa de sesión administrativa no comprobaba `force_pin_change` ni la versión de autorización del JWT; un token administrativo anterior a una revocación podía conservar acceso hasta caducar. | `api/bitrix-backfill-hours.js` incorporado en `main` el 14/08/2026. La rama exige ahora POST, identidad activa, PIN definitivo, versión vigente y empleado exactamente `Activo`; pruebas locales cubren el rechazo de método y los invariantes Auth. Efectos LIVE: `[NO DATA]`. | Verificar 405 sin mutación y 401 con sesión obsoleta en Preview antes de cambiar a `VERIFICADO`. |
...
- Reanudación obligatoria confirmada: directorio y rama correctos, árbol Git
  limpio y `main`, rama y remotos sincronizados en `5867e7d`; no existe
  `AGENTS.md` aplicable.
- Reconfirmación LIVE anónima de solo lectura: `employees`, `shifts`,
  `employee_ips` y `hk_incidencias_detalle` devolvieron HTTP 401 sin recuperar
  filas. El proyecto LIVE no figura en el conector Supabase disponible; el
  inventario administrativo agregado queda `[NO DATA]` hasta disponer de una
  vía autorizada y no se presume a partir de documentación anterior.
- Preparación local de `SEC-012`: endpoints autenticados para firmas de subida y
  descarga y eliminación backend, cliente sin URLs públicas, recodificación de
  imágenes para retirar EXIF, 10 MiB y MIME cerrados, migración privada con
  copia de policies/configuración y rollback reconstructivo. La migración se
  aplicó dos veces en PostgreSQL temporal y el rollback restauró configuración,
  policies y cardinalidad; no se modificó LIVE. `SUP-011` queda `CORREGIDO`
  pendiente de integración Preview; `SUP-012` y la autorización fina continúan
  `CONFIRMADO`/`[NO DATA]`.
- Corrección local de `ARCH-001`: `adjuntos.js` queda cargado una sola vez para
  evitar wrappers, observadores y solicitudes de firma duplicados. La prueba de
  carga única pasa; la comprobación funcional en Preview queda `[NO DATA]`.
- Diagnóstico de acceso local: abrir `index.html` mediante `file://` no puede
  alcanzar los endpoints Auth relativos y deja la aplicación sin sesión. La
  aplicación y el directorio Production respondieron HTTP 200. `index.html`
  redirige ahora ese caso al dominio canónico sin reactivar el acceso legacy.
- Integración autorizada de siete commits externos de `main` relacionados con
  horas mensuales y carga de archivos; no hubo conflictos automáticos y se
  conservaron tanto el módulo nuevo como la contención de adjuntos. La revisión
  añadió `SEC-036`: el backfill histórico queda limitado localmente a POST y a
  una sesión admin plenamente vigente. Producción y Storage LIVE no cambiaron.
