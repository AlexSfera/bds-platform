...
| UI-006 | Interfaz / Modal operativo de adjuntos | P1 | CORREGIDO | El modal abierto al pulsar el estado de una Gestión o Incidencia no montaba el visor de adjuntos. Por ello una imagen guardada no podía verse allí y tampoco aparecían `Sin adjuntos` ni los avisos de carga. | Captura de usuario del Preview del 15/08/2026 y revisión de `openItemModal()`: no existía bloque `data-adj-viewer`; los únicos consumidores estaban en detalle Dashboard y panel de Tareas. Se añade un wrapper que monta, autoriza e inicializa el visor en el modal operativo. | Verificar en Preview una gestión sin adjuntos y otra con imagen, incluida reapertura y descarga, antes de cambiar a `VERIFICADO`. |
