...
var _labChargesSaving = false; // guard contra doble-click
async function _labSaveCharges(syncrolabCashId, fecha){
  if(!_labCharges.length) return;
  if(_labChargesSaving){ console.warn('_labSaveCharges ya en curso, ignorando doble-click'); return; }
  _labChargesSaving = true;
  try {
  var ts = localTs();
  ...
  } finally { _labChargesSaving = false; }
}
...
function _labMissingTransferHistoryMessage(){
  ...
  return 'No hay efectivo traspasado anterior para '+faltan.join(' y ')+'. Registra primero un traspaso verificable con responsable.';
}
...
function openLabTraspasoModal(existingId){
  ...
  _labMissingTransferHistory = existingId
    ? { nubimed:false, virtugym:false }
    : { nubimed:true, virtugym:true };
  ...
  if(!existingId){
    ...
    dbGetAll(LAB_TABLE).then(function(rows){
      ...
      _labMissingTransferHistory = { nubimed:ultimoNub===null, virtugym:ultimoVg===null };
...
var _labSubmitting = false; // guard global contra doble-click en guardar
async function submitLabTraspaso(){
  if(_labSubmitting) return;
