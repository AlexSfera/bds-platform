...
export function canManageRecordAttachments(profile, target, record) {
  if (!profile || !target || !record) return false;
  if (isAdminProfile(profile) || isAdjuntoProfile(profile)) return true;

  // Los empleados no gestionan adjuntos, ni siquiera en registros propios.
  // La gestión queda reservada a responsables con ámbito explícito.
...
export async function requireManageableRecord(accessToken, target, profile) {
  ...
  return canManageRecordAttachments(profile, target, record) ? record : null;
}
