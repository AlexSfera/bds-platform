...
export function supervisorDepartments(profile) {
  ...
  if (profile.rol === 'jefe') {
    return AREA_GROUPS[profile.area] || (profile.area ? [profile.area] : []);
  }
...
export function targetIsInScope(actor, target) {
  ...
  const targetArea = normalizeDepartment(target && target.area);
  return !!targetArea && departments.some(dept => normalizeDepartment(dept) === targetArea);
}
