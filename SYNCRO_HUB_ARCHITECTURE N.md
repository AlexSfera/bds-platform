...
### 5.4 Roles — qué puede hacer cada usuario en Mi Turno

| Acción | Empleado | Jefe Dpto | Admin |
|---|---|---|---|
| Ver sus propios turnos anteriores | ✅ | ✅ | ✅ |
| Registrar gestión pendiente | ✅ | ✅ | ✅ |
| Registrar incidencia operativa | ✅ | ✅ | ✅ |
| Crear tarea | ✅ | ✅ | ✅ |
| **Procesar / cambiar estado de gestión** | ❌ | ✅ | ✅ |
| **Procesar / cambiar estado de incidencia** | ❌ | ✅ | ✅ |
| **Procesar / cambiar estado de tarea** | ❌ | ✅ | ✅ |

**Regla:** el empleado puede registrar pero NO procesar. Procesar (cambiar estado a "En proceso" o "Cerrada") es competencia exclusiva del jefe de departamento o administrador — desde validación o desde dashboard.

### 5.5 Gestión y tarea desde Mi Turno — independiente del cierre
...
Solo **Jefe de Departamento** y **Administrador** pueden actuar sobre gestiones, incidencias y tareas desde el modal de validación. Los empleados solo ven el estado actual.

| Elemento | Rol | Acciones disponibles |
|---|---|---|
| Gestión | Jefe Dpto / Admin | → En proceso · → Cerrar (campo "Acción tomada" obligatorio) |
| Incidencia | Jefe Dpto / Admin | → En proceso · → Cerrar (campo "Acción tomada" obligatorio) |
| Tarea | Jefe Dpto / Admin | → En proceso · → Cerrar |
| Cualquiera | Empleado | Solo lectura — sin botones de acción |
...
| Recepción | Check-ins, check-outs, ocupación, fondo inicial, total cash, total tarjeta, total transferencias, días con caja cuadrada vs descuadrada |
| Recepción SYNCROLAB | Sesiones, cobros, leads |

**KPI Recepción — detalle dashboard:**

| KPI | Fuente | Cálculo |
|---|---|---|
| Ocupación | `shifts.checkin_count` | Suma del periodo |
| Check-ins | `shifts.checkin_count` | Suma del periodo |
| Check-outs | `shifts.checkout_count` | Suma del periodo |
| Cash total | `recepcion_cash.cash_real` | Suma del periodo |
| Tarjeta total | `recepcion_cash.tpv_real` | Suma del periodo |
| Transferencias | `recepcion_cash.transferencia_banco` | Suma del periodo |
| Días cuadrados | `recepcion_cash` | COUNT WHERE dif_total = 0 |
| Días descuadrados | `recepcion_cash` | COUNT WHERE dif_total ≠ 0 con detalle |
| Fondo en caja | `recepcion_cash.fondo_real_a_traspasar` | Último registro |

### 8.4 Eliminación desde dashboard — roles

| Elemento | Quién puede eliminar |
|---|---|
| Gestión | Jefe de Departamento (solo su dpto) + Admin (todos) |
| Incidencia | Admin únicamente |
| Tarea | Admin únicamente |
| FIO | Admin únicamente |
| Cierre de caja | Admin únicamente |

**Regla:** toda eliminación requiere confirmación + motivo escrito + registro en `audit_log` antes del DELETE.
El Jefe de Departamento puede eliminar gestiones de su propio departamento — nunca de otros.

### 8.5 Filtros del dashboard
...
### 8.6 Columnas obligatorias — Incidencias y Gestiones
...
### 8.7 Columnas obligatorias — Tareas
...
### 9.5 Campo especial — Transferencias Recepción

Bajo el campo "Transferencia recibida según MEWS" añadir:
**"Última revisión transferencias bancarias"** — fecha y hora de la última vez que el campo transferencia_banco fue actualizado con valor > 0.

- Se graba automáticamente en `transferencia_banco_updated_at = localTs()` cada vez que el campo se modifica con valor > 0
- Se muestra como texto informativo: `"Última revisión: DD/MM/YYYY HH:mm"`
- No editable — solo lectura
- Propósito: saber cuándo fue la última vez que alguien revisó las transferencias en el banco (no se revisa a diario)

### 9.6 Campo Pensiones — unificado para Recepción y Sala

Eliminar campos separados "Media pensión" y "Pensión completa".
Sustituir por un único campo: **"Pensiones (excl. desayunos)"** — número de personas en régimen de media pensión o pensión completa, excluyendo desayunos.

- Campo: `pensiones_personas` (INTEGER)
- Aplica a: Recepción Hotel + Sala
- NO aplica a: Recepción SYNCROLAB

### 9.7 Tabla Supabase por departamento
